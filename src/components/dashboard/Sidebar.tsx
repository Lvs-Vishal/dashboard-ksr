...
import { LayoutDashboard, Radio, Skull, Terminal, RefreshCw } from 'lucide-react';
import { clsx } from 'clsx';
import Link from 'next/link';
import { usePathname } from 'next/navigation';

const navItems = [
    { name: 'TACTICAL MAP', icon: LayoutDashboard, path: '/' },
    { name: 'LIVE TRAFFIC', icon: Radio, path: '/traffic' },
...
export default function Sidebar() {
    ...
    const handleAttack = (type: string) => {
        ...
        publish('aegis/stats', JSON.stringify({
            packet_rate: 500,
            prob_attack: 0.95,
            type
        }));
        ...
        <aside className="w-64 border-r border-white/10 bg-black/80 backdrop-blur-md h-[calc(100vh-64px)] fixed bottom-0 left-0 flex flex-col justify-between p-4">
            ...
            <div className="space-y-6 mb-4">
                <div className="p-4 rounded-xl border border-white/10 bg-white/5">
                    <h3 className="text-xs font-bold text-neutral-500 mb-4 uppercase tracking-widest">Simulation Controls</h3>

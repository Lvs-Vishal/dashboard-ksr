...
export default function Header() {
    const { status, threatLevel } = useMqtt();
        ...
        <header className="h-16 border-b border-white/10 bg-black/50 backdrop-blur-md flex items-center justify-between px-6 sticky top-0 z-50">
            <div className="flex items-center gap-4">
                <div className={clsx(
                    ...
                    threatLevel === 'CRITICAL' ? "text-red-500 animate-pulse" : "text-green-500"
                )}>
                    {threatLevel === 'CRITICAL' ? <ShieldAlert className="w-8 h-8" /> : <Shield className="w-8 h-8" />}
                    <span>A.E.G.I.S.</span>
                </div>
                <div className="h-6 w-px bg-white/20 mx-2" />
                <span className="text-xs text-neutral-400 tracking-wider">ACTIVE DEFENSE SYSTEM</span>
            </div>

            <div className="flex items-center gap-6">
                {/* Threat Level Indicator */}
                <div className="flex items-center gap-3 bg-white/5 px-4 py-1 rounded-full border border-white/10">
                    <Activity className={clsx("w-4 h-4", threatLevel === 'CRITICAL' ? "text-red-500" : "text-green-500")} />

...
import { Thermometer, Monitor, Lightbulb, Wifi, AlertTriangle, Edit2, Check, X } from 'lucide-react';
        ...
        >
            ...
            <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-2">
                    <div className={clsx("p-2 rounded-lg", isCompromised ? "bg-red-500/20 text-red-500" : "bg-green-500/20 text-green-500")}>
                        <Icon className="w-5 h-5" />
                    </div>
                    <div>
                        <h3 className="text-sm font-bold tracking-wide text-white">{node.name}</h3>
                        {isEditing ? (
                            ...
                            <div className="flex items-center gap-2 group/ip cursor-pointer" onClick={() => setIsEditing(true)}>
                ...
                <div className={clsx("text-[10px] font-bold px-2 py-0.5 rounded border",
                    isCompromised ? "border-red-500 text-red-500 bg-red-500/10" : "border-green-500 text-green-500 bg-green-500/10"
                )}>
                    {node.status}
                </div>
            </div>
            ...
            <div className="space-y-2 font-mono text-xs">
                {node.type === 'THERMOSTAT' && (
                    <div className="flex justify-between border-t border-white/10 pt-2">
                        ...
                        <span className={clsx((node.data.temp as number) > 30 ? "text-red-400" : "text-white")}>
                            {node.data.temp as number}°C / {node.data.target as number}°C
                        </span>
                    </div>
                )}
                {node.type === 'DISPLAY' && (
                ...
                {node.type === 'LIGHT' && (
                    <div className="flex justify-between border-t border-white/10 pt-2">
                        ...
                        <span className={clsx(node.data.state ? "text-yellow-400" : "text-neutral-600")}>
            ...
            <div className="absolute top-0 right-0 p-2 opacity-50">
                {isCompromised ? <AlertTriangle className="w-12 h-12 text-red-500/10" /> : <Wifi className="w-12 h-12 text-green-500/10" />}
            </div>
        </motion.div>
    );
}
